...
Objection::Objection(Rect Box, int ID){
    Area_Objection_R=Area_limit(Box);//初始化RGB图像目标框
    ClassID=ID;
    Classname=classNamesVec[ID];//目标类别名称
    Position_Transform Start_point(array<int,2>{Area_Objection_R.x,Area_Objection_R.y}, true);//转换彩色图的坐标到深度图
...
void Objection::CheckStartPoint() {
   ...
   Pre_point.at(0)=Area_Objection_R.x+Area_Objection_R.width/2 ;
   Pre_point.at(1)=Area_Objection_R.y+Area_Objection_R.height/2;
